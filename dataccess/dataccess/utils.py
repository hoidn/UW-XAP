...
def hash_obj(obj):
    ...
    def iter_digest(to_digest):
        return obj_digest(reduce(operator.add, map(hash_obj, to_digest)))

    if (not isinstance(obj, np.ndarray)) and hasattr(obj, '__iter__') and (len(obj) > 1):
        if isinstance(obj, dict):
            return iter_digest(obj.iteritems())
        else:
        ...
        def gen_key(*args, **kwargs):
            ...
            if excluded:
                for k in merged_dict.keys():
                    if k in excluded:
                        merged_dict.pop(k)
            key = hash_obj(tuple(map(hash_obj, [args, merged_dict, closure_dict.items(), list(kwargs.iteritems())])))
            #print "key is", key
