...
def addparser_sync(subparsers):
    ...
    sync = subparsers.add_parser('sync', help = 'Launch background process to synchonize google spreadsheet data. Spreadsheet URL and sheet number(s) must be provided in config.py')
    # TODO: handle loading of url from config.py
    sync.add_argument('--url', help = 'url of the google spreadsheet')
    sync.add_argument('--sheet', '-s', type = int, default = 0, help = 'Index of the sheet (defaults to 0)')
...
def addparser_xes(subparsers):
    ...
    xes.add_argument('datapaths', nargs = '+', help = 'Paths of CSPAD data files to process into spectra and plot.')
    xes.add_argument('--pxwidth', '-p', type = int, default = 3, help = 'Pixel width of CSPAD subregion to sum.')
    xes.add_argument('--calibration', '-c', type = str, help = 'Path of data to use for calibration of the energy scale (if --energy_ref1_energy_ref2_calibration is selected but a calibration file is not provided). If not provided this parameter defaults to the first path in datapaths.')
    xes.add_argument('--subtraction', '-d', type = str, help = 'Path of a data file to use as a dark frame subtraction')
    ...
    xes.add_argument('--runlabels', '-r', type = str, default = 'labels.txt', help = 'Path to run group label input file. Defaults to labels.txt. If the file does not exist it is generated automatically (based on run data timestamps), and can subsequently be edited by the user.)')
...
def call_xes(args):
    ...
    xes_process.main(args.detid, paths, cold_calibration_path = cold_calibration_path,
        pxwidth = pxwidth,
        calib_save_path = calibration_save_path, calib_load_path =
            ...
            eltname = eltname, run_label_filename = args.runlabels)
