...
emission = {
    'Sc': [4090.6,4086.1,4460.5,4492.0],      #Sc
    'Ti': [4510.84,4504.86,4931.81,4966.0],   #Ti
    'V': [4952.2,4944.64,5427.29,5465.0],    #V
    'Cr': [5414.72,5405.509,5946.71,5989.0],  #Cr
    'Mn': [5898.75,5887.65,6490.45,6539.0],   #Mn
    'Fe': [6403.84,6390.84,7057.98,7112.0],   #Fe
    'Co': [6930.32,6915.3,7649.43,7709.0],    #Co
    'Ni': [7478.15,7460.89,8264.66,8333.0]    #Ni
    }
...
def lineout(label, detid, cencol, pxwidth = 10, default_bg = None):
    """
    Return a 1d lineout
    """
    raw = data.get_label_data(label, detid, default_bg = default_bg)
...
def get_energies(label, detid, eltname, cencol, save_path = None, **kwargs):
    """
    ...
    # element id
    kalpha = emission[eltname][0]
    kbeta = emission[eltname][2]

    spectrum = lineout(label, detid, cencol)[::-1]
...
def get_spectrum_one_label(eltname, detid, label, cold_calibration_label, \
    ...
    if calib_load_path:
        with open(calib_load_path, 'rb') as f:
            x = np.array(dill.load(f))
    elif kalpha_kbeta_calibration:
        x = get_energies(cold_calibration_label, detid, eltname, cencol, save_path = calib_save_path)
    else:
...
def plot_spectra(eltname, spectrumList, labels, name = None):
    ...
    for i in range(4):
        if elist[-1] - 50 < emission[eltname][i] < elist[0] + 50:
            plt.plot( [emission[eltname][i], emission[eltname][i]],
                [(-0.05)*max_intensity, (txtheight[i])*max_intensity],
                color = "gray")
            plt.text(emission[eltname][i]+txtshift[i], (txtheight[i])*max_intensity, lineidforplot[i], size="large")
