...
#def find_module_memoized(*args):
#    print 'imp.find_module: inputs', args
#    result = imp.find_module(*args)
#    print 'imp.find_module: return value', result
#    return result

def makeImportedModule(name, fullname, path, scope):
    ...
    def _loadModule():
        ...
        if not namedict:
            ...
            namedict['pathname'] = pathname
            namedict['desc'] = desc
            file.close()
        mod = sys.modules.get(name, None)
        if mod is None or not isinstance(mod, types.ModuleType):
            try:
                file = open(namedict['pathname'], 'U')
            except:
                file = None

            try:
                mod = imp.load_module(name, file, namedict['pathname'], namedict['desc'])
            finally:
                if file is not None:
                    file.close()
    ...
    class ModuleProxy(object):
        ...
        def __repr__(self):
            return "<moduleProxy '%s' from '%s'>" % (name, namedict['pathname'])
...
class OnDemandLoader(object):
    ...
    """
    def __init__(self, name, fullname, path, scope):
        self.name = name
        self.scope = scope
    ...
    def __init2__(self):
        file, pathname, desc = imp.find_module(self.fullname, self.path)
        self.file = file
...
#                                     self.scope)
            mod = makeImportedModule(self.name, self.fullname,
...
class OnDemandImporter(object):
    ...
    """
    __skip = ['recordclass', 'pickle', 'matplotlib', 'numpy', 'mpi4py', 'matplotlib']
    def find_module(self, fullname, path=None):
        ...
        try:
            file, pathname, desc = imp.find_module(fullname, path)
            return OnDemandLoader(origName, fullname, path, global_scope)
        except ImportError:
