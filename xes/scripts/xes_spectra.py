...
if __name__ =='__main__':
    ...
    parser.add_argument('--calibration', '-c', type = str, help = 'Path of data to use for calibration of the energy scale (if --energy_ref1_energy_ref2_calibration is selected but a calibration file is not provided). If not provided this parameter defaults to the first path in datapaths.')
    parser.add_argument('--subtraction', '-d', type = str, help = 'Path of a data file to use as a dark frame subtraction')
    parser.add_argument('--energy_ref1_energy_ref2_calibration', '-k', action = 'store_true', help = 'Enable automaic generation of energy calibration based on k alpha and k beta peak locations if --calibration_load_path is not given.')
    parser.add_argument('--eltname', '-e', default = '', help = 'Element name. This parameter is required for generating an energy scale using --energy_ref1_energy_ref2_calibration.')
    parser.add_argument('--calibration_save_path', '-s', type = str, help = 'Path to which to save energy calibration data if calibration_load_path is unspecified and --energy_ref1_energy_ref2_calibration is selected.')
